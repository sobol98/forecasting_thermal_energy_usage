...
monthly_agg_df = merged_df.groupby('month').agg(
    mean_temperature=('Średnia_godzinowa', 'mean'),
    mean_cieplo_diff=('cieplo_diff', 'mean')
).reset_index()

monthly_agg_df.to_excel('monthly_aggregated_data.xlsx', index=False)
...
x = monthly_agg_df['mean_temperature']
y = monthly_agg_df['mean_cieplo_diff']
...
plt.title("Agregacja miesięczna: Zużycie ciepła vs Średnia temperatura")
plt.xlabel("Średnia temperatura miesięczna (°C)")
