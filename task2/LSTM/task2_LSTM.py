...
class LSTMModel(nn.Module):
    ...
    def forward(self, x):
        ...
        out = out[:, -1, :]  # używamy wyjścia z ostatniego kroku czasowego
        out = self.fc1(out)
        out = self.relu(out)
        out = self.fc2(out)
        return out
...
plt.xlabel("Epoka")
plt.ylabel("Błąd (MSE)")
plt.title("Wykres uczenia modelu LSTM")
plt.legend()
plt.grid(True)
...
torch.save(model.state_dict(), f'lstm_model_{current_time}_.pth')
...
mae = mean_absolute_error(y_train_actual, predictions)
rmse = np.sqrt(mse)
print(f'MSE: {mse:.4f}')
print(f'MAE: {mae:.4f}')
